    ...
    <section className="border-t border-zinc-800/60 px-4 py-24 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="relative">
          <div
            className="absolute left-[7px] top-2 bottom-2 w-px bg-gradient-to-b from-sky-500/50 via-sky-400/30 to-transparent sm:left-3"
            aria-hidden
          />
          <ol className="flex flex-col gap-14">
            {experience.map((job, i) => (
              ...
              >
                <span
                  className="absolute left-0 top-1.5 flex h-4 w-4 items-center justify-center rounded-full border-2 border-sky-400/80 bg-zinc-950 sm:left-1"
                  aria-hidden
                >
                  ...
                  <div className="mt-1 flex flex-wrap items-baseline gap-x-2 gap-y-1 text-zinc-400">
                    {job.href ? (
                      <a
                        ...
                        className="font-medium text-zinc-300 hover:text-sky-300"
                      >
                        {job.company}
                      </a>
                    ) : (
                  ...
                  <ul className="mt-4 space-y-2 text-zinc-400">
                    {job.bullets.map((b) => (
                      <li key={b} className="flex gap-2 text-sm leading-relaxed">
                        <span className="shrink-0 text-sky-400/90" aria-hidden>
                          ▹
                        </span>
                        <span>{b}</span>
                      </li>
